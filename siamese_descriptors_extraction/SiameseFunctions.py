...
def create_base_network(input_d, hidden_layer_size):
    ...
    '''
    seq = Sequential()
    # seq.add(BatchNormalization(0, axis=1, name='major_bn'))
    for i in range(len(hidden_layer_size)):
        if i == 0:
            seq.add(Dense(hidden_layer_size[i], input_shape=(input_d,), activation='sigmoid'))
        else:
            seq.add(Dense(hidden_layer_size[i],activation='sigmoid'))
        # seq.add(Dropout(0.2))
    return seq
